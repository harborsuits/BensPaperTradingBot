...
let tradingState, requireTradingOn;
try {
  tradingState = require('./src/lib/tradingState').tradingState;
  requireTradingOn = require('./src/middleware/requireTradingOn').requireTradingOn;
} catch (error) {
  console.log('Using fallback for trading state modules');
  tradingState = {
    isPaused: () => false,
    pause: () => {},
    resume: () => {}
  };
  requireTradingOn = (req, res, next) => next();
}

// Try to import optional modules
let CONFIG = {};
let placeOrderAdapter = null;
let preTradeGate = null;

try {
  CONFIG = require('./lib/config').CONFIG || {};
} catch (error) {
  console.log('Config module not available, using defaults');
}

try {
  placeOrderAdapter = require('./lib/brokerPaper').placeOrderAdapter;
} catch (error) {
  console.log('Broker paper module not available');
}

try {
  preTradeGate = require('./lib/gate').preTradeGate;
} catch (error) {
  console.log('Gate module not available');
}
...
app.use((req, res, next) => {
  ...
  const requestId = (crypto && crypto.randomUUID) ? crypto.randomUUID() : Math.random().toString(36).slice(2);
  res.locals.requestId = requestId;
  res.locals.source = res.locals.source || 'unknown';
  res.locals.provider = res.locals.provider || 'none';
