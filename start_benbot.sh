...
open -a "Google Chrome" "http://localhost:3003?nocache=$(date +%s)"
...
# Monitor servers to ensure they stay running
...
(
    while true; do
        ...
        if ! curl -s http://localhost:3003 > /dev/null; then
            echo "$(date): UI server down, restarting..." >> "$UI_LOG"
            cd "$(dirname "$0")/new-trading-dashboard"
            npm run dev -- --port 3003 --strictPort >> "$UI_LOG" 2>&1 &
        fi
    done
