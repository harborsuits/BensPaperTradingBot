...
# Lazy TensorFlow import to avoid mutex issues
_tf = None
_keras_models = None
_keras_layers = None
...
def _get_tensorflow_imports():
    """Lazy import of TensorFlow and Keras components"""
    global _tf, _keras_models, _keras_layers, _keras_callbacks, _keras_optimizers, HAS_TENSORFLOW
    ...
    if HAS_TENSORFLOW is None:
        try:
            ...
            _tf = tf
            _keras_models = {'Sequential': Sequential, 'load_model': load_model}
            _keras_layers = {'LSTM': LSTM, 'Dense': Dense, 'Dropout': Dropout, 'BatchNormalization': BatchNormalization}
            _keras_callbacks = {'EarlyStopping': EarlyStopping, 'ModelCheckpoint': ModelCheckpoint}
            _keras_optimizers = {'Adam': Adam}

            HAS_TENSORFLOW = True
            logger.info("TensorFlow and Keras components imported successfully")
        except ImportError as e:
            logger.warning(f"TensorFlow not available. LSTM model will not work: {e}")
    ...
    return {
        'tf': _tf,
        'Sequential': _keras_models.get('Sequential') if _keras_models else None,
        'load_model': _keras_models.get('load_model') if _keras_models else None,
        'LSTM': _keras_layers.get('LSTM') if _keras_layers else None,
        'Dense': _keras_layers.get('Dense') if _keras_layers else None,
        'Dropout': _keras_layers.get('Dropout') if _keras_layers else None,
        'BatchNormalization': _keras_layers.get('BatchNormalization') if _keras_layers else None,
        'EarlyStopping': _keras_callbacks.get('EarlyStopping') if _keras_callbacks else None,
        'ModelCheckpoint': _keras_callbacks.get('ModelCheckpoint') if _keras_callbacks else None,
        'Adam': _keras_optimizers.get('Adam') if _keras_optimizers else None,
        'has_tf': HAS_TENSORFLOW
    }


# Define a dummy LSTM model class when TensorFlow is not available
class DummyLSTMModel(BaseMLModel):
        """
...
class LSTMModel(BaseMLModel):
        ...
        def _build_model(self, input_shape: Tuple[int, int]):
            ...
            """
            tf_imports = _get_tensorflow_imports()
            if not tf_imports['has_tf']:
                raise RuntimeError("TensorFlow not available")

            Sequential = tf_imports['Sequential']
            LSTM = tf_imports['LSTM']
            Dense = tf_imports['Dense']
            Dropout = tf_imports['Dropout']
            BatchNormalization = tf_imports['BatchNormalization']
        ...
        def train(self, features: pd.DataFrame, targets: pd.DataFrame = None) -> None:
            ...
            # Set up callbacks
            callbacks = [
                tf_imports['EarlyStopping'](monitor='val_loss', patience=self.patience, restore_best_weights=True),
            ]
        ...
        def save(self, filepath: Optional[str] = None) -> str:
            ...
            if self.model and tf_imports['has_tf']:
        ...
        def load(cls, filepath: str) -> 'LSTMModel':
            ...
            tf_imports = _get_tensorflow_imports()
            model_path = f"{filepath}_keras.h5"
            if os.path.exists(model_path) and tf_imports['has_tf']:
                model.model = tf_imports['load_model'](model_path)

            return model


# Factory function to return appropriate LSTM model
def create_lstm_model(*args, **kwargs):
    """Factory function that returns LSTM or dummy model based on TensorFlow availability"""
    tf_imports = _get_tensorflow_imports()
    if tf_imports['has_tf']:
        return LSTMModel(*args, **kwargs)
    else:
        logger.warning("TensorFlow not available, returning dummy LSTM model")
        return DummyLSTMModel(*args, **kwargs)