...
class PaperBroker(BrokerInterface):
    ...
    def __init__(
        ...
        self._auto_save = persist_path is not None
    ...
    def _fill_order(self, order: PaperOrder, fill_price: float) -> None:
        ...
        with self._lock:
            ...
            # Auto-save state after order fill
    ...
    def place_equity_order(
        ...
        with self._lock:
            ...
            # Auto-save state after order placement
            self._auto_save_state()
    ...
    def cancel_order(self, order_id: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Auto-save state after order cancellation
            self._auto_save_state()
    ...
    def save_state(self) -> None:
        ...
        with self._lock:
            state = {
                ...
                "open_orders": {order_id: order.to_dict() for order_id, order in self._open_orders.items()},
                "filled_orders": {order_id: order.to_dict() for order_id, order in self._filled_orders.items()},
                "canceled_orders": {order_id: order.to_dict() for order_id, order in self._canceled_orders.items()},
            ...
            try:
                ...
                logger.info(f"[PAPER] State saved to {self._persist_path}")
            except Exception as e:
                logger.error(f"[PAPER] Failed to save state: {str(e)}")
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            self._base_currency = state.get("base_currency", self._base_currency)
            self._last_order_id = state.get("last_order_id", 0)
            ...
            for symbol, pos_data in state.get("positions", {}).items():
                ...
                position.unrealized_pl = pos_data.get("unrealized_pl", 0.0)
                position.realized_pl = pos_data.get("realized_pl", 0.0)
                position.current_price = pos_data.get("current_price", 0.0)
            ...
            for order_id, order_data in state.get("open_orders", {}).items():
                order = PaperOrder(
                    order_id=order_id,
                ...
                order.fill_price = order_data.get("fill_price")
                order.average_price = order_data.get("average_price")
                order.fees = order_data.get("fees", 0.0)
                self._open_orders[order_id] = order

            for order_id, order_data in state.get("filled_orders", {}).items():
                order = PaperOrder(
                    order_id=order_id,
                    symbol=order_data["symbol"],
                    side=order_data["side"],
            ...
            for order_id, order_data in state.get("canceled_orders", {}).items():
                order = PaperOrder(
                    order_id=order_id,
                    symbol=order_data["symbol"],
                    side=order_data["side"],
                ...
                order.status = "canceled"
                if order_data.get("canceled_at"):
                    order.canceled_at = datetime.fromisoformat(order_data["canceled_at"])
                self._canceled_orders[order_id] = order
            ...
            # Restore models
            self._slippage_model = state.get("slippage_model", {"type": "none"})
            self._commission_model = state.get("commission_model", {"type": "none"})

            saved_at = state.get("saved_at", "unknown")
            logger.info(f"[PAPER] State loaded from {self._persist_path} (saved at {saved_at})")

        except Exception as e:
            logger.error(f"[PAPER] Failed to load state: {str(e)}")

    def _auto_save_state(self) -> None:
        """Auto-save state after significant changes."""
        if self._auto_save:
            self.save_state()
