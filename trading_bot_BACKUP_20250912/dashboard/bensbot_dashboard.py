...
if mongo_available:
    try:
        ...
        mongo_available = False
        st.sidebar.error(f"MongoDB connection error - using demo data")
else:
    st.sidebar.warning("MongoDB not installed - using demo data")
...
def generate_mock_data():
    ...
    # Generate price history for equity chart
    days = 30
    dates = [(datetime.now() - timedelta(days=i)).strftime('%Y-%m-%d') for i in range(days, 0, -1)]
    
    # Generate random equity curve with upward trend
    base = 100000  # Starting equity
    equity = [base]
    for i in range(1, days):
        change = random.uniform(-1000, 1500)  # Random daily change
...
def get_portfolio_data():
    if mongo_available:
        try:
            ...
            # Get equity history
            # For simplicity using mock data for now - would need to implement in real system
            days = 30
            dates = [(datetime.now() - timedelta(days=i)).strftime('%Y-%m-%d') for i in range(days, 0, -1)]
...
# Function to place a buy order (simulation for UI)
def place_buy_order(symbol, quantity):
    ...
    # Here we would integrate with the trading engine
    # For now just show a success message
    return {
        "status": "success", 
        "message": f"Buy order placed for {quantity} {symbol} @ ${current_price:.2f}",
        "order_id": f"order_{random.randint(1000, 9999)}"
    }

# Function to place a sell order (simulation for UI)
def place_sell_order(symbol, quantity):
    ...
    return {
        "status": "success", 
        "message": f"Sell order placed for {quantity} {symbol} @ ${current_price:.2f}",
        "order_id": f"order_{random.randint(1000, 9999)}"
    }
...
if account_type == "Live Trading":
    st.sidebar.warning("Live trading is not available in this simplified dashboard. Showing paper trading data.")

# System status
system_status = "Active" if mongo_available else "Demo Mode"
st.sidebar.success(f"System Status: {system_status}")
...
with st.sidebar.expander("Manual Trading", expanded=False):
    # Symbol input
    ...
    if symbol:
        ...
        if price:
            ...
            st.info(f"Total value: ${price * quantity:.2f}")
        else:
            st.warning(f"Could not get price for {symbol}")
    ...
    with col1:
        if st.button("Buy", use_container_width=True):
            result = place_buy_order(symbol, quantity)
    ...
    with col2:
        if st.button("Sell", use_container_width=True):
            result = place_sell_order(symbol, quantity)
            if result["status"] == "success":
                st.success(result["message"])
            else:
                st.error(result["message"])
...
# Footer
st.caption(f"BensBot Trading Dashboard - Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
