#!/bin/bash
# BenBot Complete Launcher - Uses complete_server.py for full API support
...
{
  echo "==============================================="
  echo "🚀 BenBot Trading Dashboard Launch (Complete)"
  date
  echo "==============================================="
} > "$LOG"
...
# Kill any existing processes
{
  ...
  pkill -f "python3 complete_server.py" 2>/dev/null || true
...
# Start API
{
  echo "Starting Complete API server with: $PYTHON"
  cd "$API_DIR" || exit 1
  
  # Use complete_server.py
  if [ -f complete_server.py ]; then
    echo "Using complete_server.py"
    nohup "$PYTHON" complete_server.py >> "$LOG" 2>&1 &
  else
    # Fall back to basic_server.py
    echo "Complete server not found, falling back to basic_server.py"
...
# Ensure UI env
{
  ...
  echo "VITE_ADMIN_PASSWORD=changeme" >> .env.local
  echo "VITE_API_KEY_PRIMARY=localdev-123456" >> .env.local
