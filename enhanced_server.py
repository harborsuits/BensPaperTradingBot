...
# Enhanced server that implements all endpoints needed by the React dashboard
class EnhancedHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Strategies endpoint - handle both /api/v1/strategies and /strategies
        ...
        # Risk status endpoint - handle both /api/v1/risk/status and /safety/status
        ...
        # Decisions endpoint
        elif path == '/decisions' or path == '/decisions/latest':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.send_cors_headers()
            self.end_headers()
        ...
        # Root endpoint
            ...
            self.send_cors_headers()
            self.end_headers()
            response = {'message': 'Enhanced API Server', 'status': 'running'}
            self.wfile.write(json.dumps(response).encode())
