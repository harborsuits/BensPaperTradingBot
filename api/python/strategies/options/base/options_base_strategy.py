...
from trading_bot.strategies.base.base_strategy import Strategy
...
class OptionsBaseStrategy(Strategy):
    """Base class for all options trading strategies."""
    
    def __init__(self, name: str, parameters: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None):
        """
        Initialize the options base strategy.
    ...
    def validate_metadata(self) -> None:
        ...
        required_fields = ['asset_class', 'strategy_type', 'compatible_market_regimes', 'timeframe']
        for field in required_fields:
            if field not in self.metadata:
                logger.warning(f"Missing required metadata field: {field}")
                
        # Validate market regimes
        if 'compatible_market_regimes' in self.metadata:
            regimes = self.metadata['compatible_market_regimes']
            if not isinstance(regimes, list):
                logger.warning("compatible_market_regimes must be a list")
            else:
                for regime in regimes:
                    try:
                        MarketRegime(regime)
                    except ValueError:
                        logger.warning(f"Invalid market regime in metadata: {regime}")
    ...
    def is_compatible_with_regime(self, regime: MarketRegime) -> bool:
        ...
        """
        if 'compatible_market_regimes' not in self.metadata:
            return True  # If not specified, assume compatible with all regimes
            
        return regime.value in self.metadata['compatible_market_regimes']
        
    def get_regime_compatibility_score(self, regime: MarketRegime) -> float:
        """
        Get compatibility score for a market regime.
        ...
        """
        if 'regime_compatibility_scores' not in self.metadata:
            return 0.5  # Default to neutral compatibility
            
        return self.metadata['regime_compatibility_scores'].get(regime.value, 0.5)
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> 'OptionsBaseStrategy':
